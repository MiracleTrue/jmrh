...
Route::post('login/submit', 'IndexController@LoginSubmit');/*登录提交 */
...
Route::group(['middleware' => ['WebLoginAndPrivilege']], function ()
{
    Route::group(['group' => '用户中心', 'identity' => [\App\Models\User::PLATFORM_ADMIN, \App\Models\User::SUPPLIER_ADMIN, \App\Models\User::ARMY_ADMIN]], function ()
    {
        ...
        Route::post('product/ajax/list', 'ProductController@ProductAjaxList')->name('获取商品列表');/*获取商品列表*/
    });
    ...
    Route::group(['group' => '平台', 'identity' => [\App\Models\User::PLATFORM_ADMIN]], function ()
    {
        ...
        Route::post('platform/send/army', 'PlatformController@SendArmy')->name('发货到军方');/*发货到军方*/
    });
    ...
    Route::group(['group' => '军方', 'identity' => [\App\Models\User::ARMY_ADMIN]], function ()
    {
        ...
        Route::post('army/need/release', 'ArmyController@NeedRelease')->name('发布需求');/*军方发布需求*/
        Route::post('army/need/edit', 'ArmyController@NeedEdit')->name('修改需求');/*军方修改需求*/
        Route::post('army/need/delete', 'ArmyController@NeedDelete')->name('删除需求');/*军方删除需求*/
        Route::post('army/confirm/receive', 'ArmyController@ConfirmReceive')->name('确认收货');/*军方删除需求*/
    });
    ...
    Route::group(['group' => '供应商', 'identity' => [\App\Models\User::SUPPLIER_ADMIN]], function ()
    {
        ...
        Route::post('supplier/offer/submit', 'SupplierController@OfferSubmit')->name('报价');/*报价提交*/
        Route::post('supplier/send/goods', 'SupplierController@SendGoods')->name('配货');/*供应商配货*/
    });
    ...
    Route::group(['group' => '用户管理', 'identity' => [\App\Models\User::ADMINISTRATOR]], function ()
    {
        ...
        Route::post('user/check/name', 'UserController@UserCheckName')->name('检测用户名占用');/*检测用户名占用*/
        Route::post('user/add', 'UserController@UserAdd')->name('新增用户');/*新增用户*/
        Route::post('user/edit', 'UserController@UserEdit')->name('修改用户');/*修改用户*/
        Route::post('user/enable', 'UserController@UserEnable')->name('启用用户');/*启用用户*/
        Route::post('user/disable', 'UserController@UserDisable')->name('禁用用户');/*禁用用户*/
    });
    ...
    Route::group(['group' => '商品管理', 'identity' => [\App\Models\User::PLATFORM_ADMIN]], function ()
    {
        ...
        Route::post('category/add', 'ProductController@CategoryAdd')->name('新增商品分类');/*新增商品分类*/
        Route::post('category/edit', 'ProductController@CategoryEdit')->name('修改商品分类');/*修改商品分类*/
        Route::post('category/delete', 'ProductController@CategoryDelete')->name('删除商品分类');/*删除商品分类*/
        ...
        Route::post('product/delete', 'ProductController@ProductDelete')->name('删除商品');/*删除商品*/
    });
