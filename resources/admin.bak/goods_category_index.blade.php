	...
	<div class="page-container">
		<div class="pd-5 mb-10 bg-1 bk-gray prompt">
			...
			<div class="pl-30 pr-30 cl">
				<div class="col-sm-12 l"><i class="Hui-iconfont c-primary pr-5">&#xe677;</i><span class="c-primary f-12">该页面展示所有商品分类</span></div>
		...
		<div class="container-fluid cl">
			<div id="goods_tree" class="ztree f-l col-xs-3"></div>
			<iframe class="f-r col-xs-9" id="goods_tree_frame" name="goods_tree_iframe" frameborder=0 scrolling=auto style="min-height: 540px" src="{{action('Admin\GoodsController@CategoryView')}}"></iframe>
		</div>
	</div>
...
var zTreeSetting = {
	...
	callback: {
		beforeClick: function(treeId, treeNode) {
			var zTree = $.fn.zTree.getZTreeObj("goods_tree");
			$('#goods_tree_frame').attr("src","{{action('Admin\GoodsController@CategoryView')}}" + "/" + treeNode.id);
		},
		beforeRemove :function (treeId, treeNode)
		{
			...
			layer.confirm('{{__('admin.deleteConfirm')}}',function(index){
				$.ajax({
					type: 'POST',
					url: '{{action('Admin\GoodsController@CategoryDeleteOne')}}',
					data:{
						"_token":"{{csrf_token()}}",
...
$(document).ready(function(){
	var zTree = $.fn.zTree.init($("#goods_tree"), zTreeSetting, zTreeNodes);
	...
	$('#ztree_open_all').click(function()
	{
		if(!expandAll_index)
		{
			$.fn.zTree.getZTreeObj("goods_tree").expandAll(true);
			expandAll_index = true;
		}
		else
		{
			$.fn.zTree.getZTreeObj("goods_tree").expandAll();
