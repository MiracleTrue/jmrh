	...
	<div class="page-container">
		...
		<div class="text-c">
			...
			<div class="formControls col-xs-3 col-sm-3">
				<div>
					<input class="input-text" id="parent_text"   name="parent_id" type="text" readonly value="{{$category_info['name'] or '全部品牌'}}"/>
				</div>
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l">
				<a class="btn btn-primary radius"onclick="article_add('添加品牌','{{action('Admin\GoodsController@BrandView')}}')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加品牌</a>
			</span>
			<span class="r">{{__('admin.countData')}}：<strong>{{$brand_list->total()}}</strong><i class="Hui-iconfont"> &#xe6c1;</i></span>
		</div>

		<table class="table table-border table-bordered table-bg table-hover table-sort">
			<thead>
			<tr>
				<th scope="col" colspan="20">品牌列表</th>
			</tr>
			<tr class="text-c">
			...
			@foreach($brand_list as $item)
				<tr class="text-c">
					...
					<td>{{$item['brand_id']}}</td>
					<td><img width="100%" src="{{$item['brand_logo']}}"></td>
					<td>{{$item['brand_sort']}}</td>
					<td>{{$item['brand_name']}}</td>
					<td class="text-l">{{$item['brand_description']}}</td>
					<td>{{$item['goods_category']['name']}}</td>
					<td>{{$item['merchant_info']['shop_name']}}</td>
					<td class="f-14 td-manage">
						<a style="text-decoration:none" class="ml-5" onClick="article_add('编辑品牌','{{action('Admin\GoodsController@BrandView',$item['brand_id'])}}')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
						<a style="text-decoration:none" class="ml-5" onClick="article_del(this,'{{$item['brand_id']}}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
					</td>
				</tr>
			@endforeach
			</tbody>
		</table>
		@include('admin.include.inc_pagination',['pagination'=>$brand_list])
	</div>
	...
	<script type="text/javascript">
		...
		var zTreeSetting = {
			...
			callback: {
				beforeClick: function(treeId, treeNode) {
					location.replace("{{action('Admin\GoodsController@BrandIndex')}}" + "/" + treeNode.id);
				}

			}
		};
		var zTreeNodes =[
			{ id:0, name:"全部品牌", open:true},
		...
		function article_del(obj,id){
			layer.confirm('确认要删除吗？',function(index){
				$.ajax({
					url: '{{action('Admin\GoodsController@BrandDeleteOne')}}',
